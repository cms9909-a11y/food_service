...
> **이미 예전 버전을 실행해 둔 프로젝트라면** 그냥 다시 실행하면 됩니다.
> `search_logs` 가 있어도 부족한 열(`found`)과 제약을 알아서 채우며,
> **쌓여 있던 로그는 지우지 않습니다.** (`seed.sql` 과 달리 `truncate` 를 하지 않습니다.)
>
> `column "found" ... does not exist` 오류가 났다면 예전 버전으로 만든 테이블입니다.
> 그 테이블은 `kind` 에 `'feedback'` 을 허용하지 않아 **만족도 응답이 저장되지 않고
> 있었으니**, 최신 `analytics.sql` 을 한 번 실행해 주세요.
